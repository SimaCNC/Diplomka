...
//KONFIGURACNI FUNKCE PRI SPUSTENI
int BME280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)
{
	...
	// zapis tlaku a teploty oversambling s modem do 0xF4 ( and temp oversampling along with mode to 0xF4)
	...
	return 0;  //Success
}/* BME280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)*/
...
// Precist trimming parametry ulozene v NVM ROM (Read the Trimming parameters saved in the NVM ROM of the device)
static int TrimRead(void)
{
	uint8_t trimdata[32];

	// Precist NVM od 0x88 do 0XA1 (Read NVM from 0x88 to 0xA1 (25 bytes))
	if (HAL_I2C_Mem_Read(&BME280_I2C, BME280_ADDRESS, 0x88, 1, trimdata, 25, HAL_MAX_DELAY) != HAL_OK)
	{
			return 1;  //error NUM for debug
	}

	// Precist NVM od 0xE1 do 0xE7 (Read NVM from 0xE1 to 0xE7 (7 bytes))
	if (HAL_I2C_Mem_Read(&BME280_I2C, BME280_ADDRESS, 0xE1, 1, trimdata + 25, 7, HAL_MAX_DELAY) != HAL_OK)
	{
		return 2;  //error NUM for debug
	}

	// Teplotni koeficienty
...
static int BMEReadRaw(void)
{
	...
	// Zkontrolovat pred ctenim (Check the chip ID before reading)
	HAL_I2C_Mem_Read(&hi2c1, BME280_ADDRESS, ID_REG, 1, &chipID, 1, 1000);

	if (chipID == 0x60)
	{
		// Precist registry 0xF7 to 0xFE (Read the Registers 0xF7 to 0xFE)
		HAL_I2C_Mem_Read(&BME280_I2C, BME280_ADDRESS, PRESS_MSB_REG, 1, RawData, 8, HAL_MAX_DELAY);

		/* Vypocitat raw data pro parametry
		 * tlak a teplota jsou v 20 bit formatu a vlhkosti v 16 bit formatu
...
/************* kompenzacni vypocet dle datasheetu (COMPENSATION CALCULATION AS PER DATASHEET (page 25)) **************************/
